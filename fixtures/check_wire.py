...
def main():
    for name in ("demo-state.json",):
        ...
        assert doc["lastHeard"] == doc["generatedAt"]
        assert doc["property"]["lastHeard"] == doc["generatedAt"]
        assert all(c["kind"] == "custom" for c in doc["cards"])
        assert all(c.get("lastUpdated") for c in doc["cards"])
