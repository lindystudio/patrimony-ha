...
1. **Add-on store → ⋮ → Repositories** → add this add-on repository URL (or copy `addons/patrimony_collection` into `/addons` for a local add-on).
2. Install **Patrimony Collection**. Start it. Open the UI (Ingress).
3. Confirm house fields (property id, display name, location, timezone). Search entities, pick a card kind, save.
