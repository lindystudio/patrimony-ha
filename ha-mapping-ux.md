...
## House health last-heard (0.4.29)

Locked 18 Sep 2026: PresentationDocument is additive (schemaVersion stays 1; unknown keys ignored). Every card still emits `kind: "custom"` (title is the label).

- Root `lastHeard` — ISO-8601 UTC (`…Z`). Same instant as `generatedAt`. **iOS house-health clock** (prefer this field).
- `property.lastHeard` — same stamp, mirrored onto `property` for wallet identity.
- `card.lastUpdated` — ISO-8601 UTC (`…Z`). Newest `item.updatedAt` on that card; empty card uses `generatedAt`.
...
Locked 19 Sep 2026: PresentationDocument is additive (schemaVersion stays 1; unknown keys ignored). Root `events` is a curated Activity feed from **mapped** item changes only — not HA Logbook.

- `{ "at": "<ISO-8601 UTC>", "message": "<short human line>" }`
- Cap 8, newest first. Omit or `[]` when empty.
- Append when the presented value/label for a mapped item would change on the pass.
- Prefer the card item label / friendly title. Never `entity_id`, area ids, IP, hostname, `hek_`, or raw attributes.
- Included on every REST `/api/patrimony_collection/state` and WS `patrimony/state` snapshot.
