...
def build_presentation_document(
    ...
    *,
    ...
    for card_id, mappings in grouped.items():
        ...
        kind = meta.get("kind") if meta.get("kind") in KINDS else "custom"
        title = (meta.get("title") or "").strip() or KIND_DEFAULT_TITLES.get(kind, "Custom")
        try:
        ...
        for mapping in mappings:
            item = _build_item(mapping, kind, hass)
            if item:
                items.append(item)
