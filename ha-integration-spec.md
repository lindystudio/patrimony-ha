...
| `lastHeard` | string ISO-8601 UTC (`…Z`) | no — additive; same instant as `generatedAt`. iOS house-health clock |
| `property` | object | yes |
| `cards` | array | yes (empty allowed) |
| `events` | array | no — additive; omit or `[]` when empty. Cap 8, newest first. Mapped activity only |
...
| `locationLabel` | string | no — free-text place only. Never street / city / country fields |
| `timezone` | string IANA | yes |
| `localTime` | string `HH:mm` | no — additive; house clock |
| `lastHeard` | string ISO-8601 UTC (`…Z`) | no — additive; snapshot assembly time. iOS house-health clock |

Do not emit: `latitude`, `longitude`, `lat`, `lon`, `coordinate`, `coordinates`, `region`, `map`, `entity_id`, `haHostname`, `street`, `city`, `country`, `address`.
...
|---|---|---|
| `id` | string UUID | yes |
| `kind` | `custom` | yes — title is the label; always `custom` so schema 1.0 does not bump |
| `title` | string | yes |
| `priority` | integer | yes |
| `items` | array | yes |
| `lastUpdated` | string ISO-8601 UTC (`…Z`) | no — additive; newest `item.updatedAt` on that card (else `generatedAt`) |
...
```json
{
  ...
  "event": {
    ...
    "data": { "schemaVersion": 1, "generatedAt": "…", "lastHeard": "…", "property": {}, "cards": [], "events": [] }
  }
}
```
...
House: Demo Home, timezone `UTC`, `property.id` `00000000-0000-4000-8000-000000000001`. Cards are labels (`kind: "custom"`). `property.lastHeard` matches `generatedAt`. Each card has `lastUpdated`. Root `events` is `[]` (no mapped activity in the fixture). Includes:

- Network / Guest Wi-Fi `true` bool `ok`
- Climate / Indoor `21.5` `°C` `ok`
- Alarm / Alarm `Armed away` enum `ok`
- Weather: Outdoor `18` `°C` `ok`, Condition `Cloudy` enum `ok` (source: house `weather.*`, typically Met.no — not in the fixture)
